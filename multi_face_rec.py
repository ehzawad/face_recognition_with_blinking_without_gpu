...
def capture(read_frame_list, Global, worker_num):
    ...
    video_capture = cv2.VideoCapture("rtsp://admin:admin321!!@192.168.10.33:554/ch01/0")
...
if not os.path.exists("encodings.pkl"):
    ...
    for i in range(len(my_list)):
        if(my_list[i]!=".ipynb_checkpoints"):
            ...
            Global.known_face_names.append(my_list[i])

    with open('encodings.pkl','wb') as f:
        pickle.dump([Global.known_face_encodings,Global.known_face_names], f)
else:
    with open('encodings.pkl', 'rb') as f:
        Global.known_face_encodings ,Global.known_face_names = pickle.load(f)
    ...
    while not Global.is_exit:
        while Global.write_num != last_num:
            ...
            cv2.putText(fps, "FPS: {:.1f}".format(fps), (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
